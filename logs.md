...
## 2026-07-26 - Clean session file from git history + formalize commit conventions

- **Fix** - `session-ses_079a.md` was accidentally committed and then deleted in a subsequent commit, but remained in git history.
- **Actions**:
  - Added `session-ses_*.md` to `.gitignore`.
  - Used `git filter-repo --path session-ses_079a.md --invert-paths` to scrub the file from all commits.
  - Force-pushed rewritten history to `master`.
  - Added commit conventions section to `AGENTS.md` (Conventional Commits, imperative present tense, capitalized after prefix).
  - Removed duplicate "Commit Conventions" section from local-only `CONTEXT.md`.
- **Verification** - Working tree clean, `origin/master` matches local. 106 tests still pass, typecheck clean.
