...
describe("event system", () => {
  ...
  it("returns false when no handler is registered for the event", async () => {
    ...
    expect(ctx.hasListeners("email.received")).toBe(false);
  });
  ...
  it("rejects empty event names at runtime", async () => {
    ...
    expect(() => ctx.on("" as never, () => Promise.resolve(true))).toThrow(
      "Event name is required.",
    );
    await expect(ctx.emit("" as never, {} as never)).rejects.toThrow(
      "Event name is required.",
    );
  });
});